...
export async function parseVault(config: VaultConfig): Promise<GraphData> {
	...
	for (const filePath of mdFiles) {
		...
		for (const target of allWikilinks) {
			const resolvedId = resolveWikilink(target, noteId, fileIndex, config);
 ...
 */
function buildFileIndex(files: string[], vaultPath: string): Map<string, string> {
...
function extractWikilinksFromFrontmatter(fm: Record<string, unknown>): string[] {
	const results: string[] = [];
	...
	if (tagMode === 'flat') {
		...
		for (let i = 0; i < parts.length; i++) {
			...
			if (i > 0) {
				...
				if (!exists) {
					links.push({ source: tagId, target: parentId, type: 'tag-hierarchy' });
				}
			}
		}
	}
