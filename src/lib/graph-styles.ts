...
export function getNodeAlpha(node: GraphNode, config: StyleConfig): number {
	...
	if (!selectedNode || !globalEnabled || selectedNode.community === undefined) {
		return 1.0;
	}
...
export function getLinkAlpha(link: any, config: StyleConfig): number {
	...
	const isInternal = source.community === highlightCommunity && target.community === highlightCommunity;
