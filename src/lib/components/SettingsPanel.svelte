<script lang="ts">
	import { GLOBAL_ALGORITHMS } from '$lib/algorithms/index.js';
...
<div class="settings-panel" class:collapsed>
	<button class="toggle-btn" onclick={() => (collapsed = !collapsed)} aria-label="Toggle settings">
		{#if collapsed}
	...
	{#if !collapsed}
		<div class="panel-content">
			...
			<div class="field">
				...
				<input id="vault-path" type="text" bind:value={vaultPath}
					placeholder="/path/to/obsidian/vault"
					onkeydown={(e: KeyboardEvent) => { if (e.key === 'Enter') onLoadVault(); }} />
			</div>

			<div class="field">
				<label>Link Resolution</label>
				<div class="toggle-group">
					<button class:active={linkMode === 'auto'} onclick={() => (linkMode = 'auto')}>Auto</button>
					<button class:active={linkMode === 'absolute'} onclick={() => (linkMode = 'absolute')}>Absolute</button>
					<button class:active={linkMode === 'relative'} onclick={() => (linkMode = 'relative')}>Relative</button>
				</div>
			</div>
			...
			<div class="field">
				...
				<div class="toggle-group">
					<button class:active={tagMode === 'flat'} onclick={() => (tagMode = 'flat')}>Flat</button>
					<button class:active={tagMode === 'hierarchical'} onclick={() => (tagMode = 'hierarchical')}>Hierarchical</button>
				</div>
			</div>

			<button class="load-btn" onclick={onLoadVault} disabled={loading || !vaultPath}>
				{#if loading}<span class="spinner"></span> Loading…{:else}Load Vault{/if}
			</button>
			...
			<div class="field">
				...
				<select class="algo-select" bind:value={globalAlgorithmId} onchange={() => { if(globalEnabled) onApplyGlobal(); }}>
					{#each GLOBAL_ALGORITHMS as algo}
						<option value={algo.id} title={algo.description}>{algo.name}</option>
					{/each}
				</select>
			</div>

			<button class="analytics-btn" class:active={globalEnabled} onclick={onToggleGlobal} disabled={nodeCount === 0}>
			...
			{#if nodeCount > 0}
				<hr class="divider" />
				<div class="stats">
					<div class="stat"><span class="stat-value">{nodeCount}</span><span class="stat-label">Nodes</span></div>
					<div class="stat"><span class="stat-value">{linkCount}</span><span class="stat-label">Links</span></div>
					{#if communityCount > 0}
						<div class="stat"><span class="stat-value">{communityCount}</span><span class="stat-label">Communities</span></div>
					{/if}
				</div>
			{/if}
...
<style>
	...
	.settings-panel.collapsed { width: 48px; overflow: hidden; }
	...
	.collapsed .toggle-btn { top: 10px; right: 10px; }
	...
	.load-btn {
		...
		border-radius: 10px; color: rgba(200, 210, 255, 0.95);
		font-size: 13px; font-weight: 600; font-family: 'Inter', sans-serif;
		cursor: pointer; display: flex; align-items: center;
		justify-content: center; gap: 8px; transition: all 0.2s;
	}
	.load-btn:hover:not(:disabled) {
		background: linear-gradient(135deg, rgba(100, 120, 255, 0.35), rgba(140, 100, 255, 0.35));
	...
	.analytics-btn:disabled { opacity: 0.3; cursor: not-allowed; }
	...
	.hint {
		...
		color: rgba(255, 255, 255, 0.25); line-height: 1.5; font-style: italic;
	}
