    ...
    for (const startNode of nodes) {
      ...
      for (let step = 1; step < walkLength; step++) {
        ...
        if (prev === null) {
          ...
          // Biased transition
          next = sampleBiasedNeighbor(graph, prev, current, neighbors, p, q);
        }
  ...
  for (let iter = 0; iter < fullOptions.iterations; iter++) {
    ...
    for (const walk of walks) {
      for (let i = 0; i < walk.length; i++) {
        ...
        const end = Math.min(walk.length - 1, i + fullOptions.windowSize);
        ...
        for (let j = start; j <= end; j++) {
          ...
          // Positive sample
          updateSGNS(targetEmbed, contextEmbed, targetIdx, contextIdx, 1, alpha, dim, fullOptions.negativeSamples, n);
        }
      }
    }
    // Decay learning rate?
