...
export const JaccardAlgorithm: LocalAlgorithm = {
	...
	description: 'Measures the similarity of two nodes by dividing their shared neighbors by their total unique neighbors.',
	execute: (data: GraphData, selectedNodeId: string): AnalyticsResult[] => {
		...
		graph.forEachNode((nodeId) => {
			...
			if (shared > 0) {
				const totalUnique = selectedNeighbors.size + nodeNeighbors.length - shared;
